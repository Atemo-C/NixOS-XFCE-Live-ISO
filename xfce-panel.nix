{ config, ... }: { home-manager.users.nixos.xfconf.settings.xfce4-panel = {

	# Defines the top panel.
	"panels" = [ 1 ];

	# Set the preference for dark mode for the top panel.
	# This is not strictly necessary as it is generally detected by the theme, but just in case.
	"panels/dark-mode" = true;

	# Settings for the top panel.
	"panels/panel-1/icon-size" = 16;           # Sets the icon size for elements in the pannel.
	"panels/panel-1/length" = 100;             # Makes the pannel fill the current monitor.
	"panels/panel-1/position" = "p=6;x=0;y=0"; # Puts the pannel on the top.
	"panels/panel-1/position-locked" = true;   # Locks the pannel's position.
	"panels/panel-1/size" = 26;                # Sets the size of the pannel.
